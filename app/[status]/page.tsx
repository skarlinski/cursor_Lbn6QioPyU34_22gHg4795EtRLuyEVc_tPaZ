...
export default async function Info({ params }: { params: { status: string } }) {
  ...
  // Determine base status (built-in) if present
  const baseStatus = (statuses as any)[code];

  // Probe server for custom statuses (small JSON, avoids fetching full image)
  let hasCustom = false;
  try {
  ...
  const statusInfoHTML = baseStatus ? await getStatusInfo(code) : '<p>This is a custom status cat, uploaded by a user.</p>';
          ...
          <StatusDescription>
            ...
            <div className="mt-4 text-center">
                <Link href={`/upload?status=${statusObj.code}`} className="text-interactive hover:underline">
                    Missing a cat for this status? Upload one!
                </Link>
            </div>
          </StatusDescription>
        </section>
